...
class LaserScan:
    ...
    def open_scan(self, filename,coordinate=None,features=None):
        ...
        if type(coordinate) != type(None):
            scan = np.hstack((coordinate,features.reshape((-1,1))))
        else:
...
class SemLaserScan(LaserScan):
    ...
    def open_label(self, filename,labels=None):
        """ Open raw scan and fill in attributes
        """
        # check filename is string
        if not isinstance(filename, str):
...
if __name__ == "__main__":
    ...
    pointC = np.fromfile("/mnt/storage/dataset/semanticKitti/dataset/sequences/00/velodyne/000000.bin",dtype=np.float32).reshape((-1,4))    
    label = np.fromfile("/mnt/storage/dataset/semanticKitti/dataset/sequences/00/labels/000000.label",dtype=np.int32)
    
    pcWithlabel = np.hstack((pointC,label.reshape(-1,1)))
    
    point_1 = pcWithlabel[pcWithlabel[:,0]>=0]
    point_1 = point_1[point_1[:,1]>=0]

    point_2 = pcWithlabel[pcWithlabel[:,0]<0]
    point_2 = point_2[point_2[:,1]>=0]

    point_3 = pcWithlabel[pcWithlabel[:,0]<0]
    point_3 = point_3[point_3[:,1]<0]

    point_4 = pcWithlabel[pcWithlabel[:,0]>0]
    point_4 = point_4[point_4[:,1]<0]

    points = [point_1,point_2,point_3,point_4]
    for i in range (len(points)):
        points[i][:,-1].astype(np.int32).tofile(f"/home/guye/RidarS/RangeView/{i:06}.label")
        points[i][:,:-1].astype(np.float32).tofile(f"/home/guye/RidarS/RangeView/{i:06}.bin")

    # scan = LaserScan(True,64,2048,3.0,-25.0,False,False,False,False)
